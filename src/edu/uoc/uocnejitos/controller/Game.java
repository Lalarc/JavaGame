...
public class Game {
    ...
    private void incNumMoves(Move move, boolean isBunny) {
    	...
    	if(isBunny) {
    		...
    		/*int rowPiece = move.getRowStart();
        	int columnPiece = move.getColumnStart();
        	int checkPosition = 0;
    		for (int j = 0; j < rowPiece; j++) {
    			checkPosition += 5;
    		}
    		for (int i = 0; i < columnPiece; i++) {
    				checkPosition++;
    		}
    		//Symbol moveCheckS = Symbol.GRASS;
    		if(Symbol.FOX_HEAD_LEFT == level.getBoard1D().get(checkPosition).getSymbol());{
    			FoxHead checkMove = new FoxHead(move.getStart(),FoxDirection.LEFT);
    			
    		}
    		if(Symbol.FOX_HEAD_RIGHT == level.getBoard1D().get(checkPosition).getSymbol());{
    			
    		}
    		if(Symbol.FOX_HEAD_UP == level.getBoard1D().get(checkPosition).getSymbol());{
    			
    		}
    		if(Symbol.FOX_HEAD_DOWN == level.getBoard1D().get(checkPosition).getSymbol());{
